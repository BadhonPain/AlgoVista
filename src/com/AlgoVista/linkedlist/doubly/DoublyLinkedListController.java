...
public class DoublyLinkedListController {
    ...
    @FXML
    private Label dynamicComplexityLabel;
    @FXML
    private VBox stepLogContainer;
    ...
    public void initialize() {
        ...
        listContainer.sceneProperty().addListener((obs, oldScene, newScene) -> {
            if (newScene != null) {
                ShortcutManager.register(newScene, null, null, () -> clear(), () -> backToMenu());
            }
        });
    }
    ...
    private void append() {
        ...
        new Thread(() -> {
            try {
                ...
                if (size > 0)
                    logStep("Found Tail. Setting Tail's next to new node, and new node's prev to Tail...");
                else
                    logStep("List is empty. Setting Head & Tail to new node...");
                Thread.sleep((long) ((800) * com.AlgoVista.utils.SettingsManager.getSleepMultiplier()));
            ...
            try {
                ...
                if (idx > 0 && curr == null) {
                    ...
                    javafx.application.Platform
                            .runLater(() -> showAlert("Invalid Index", "Cannot insert at index " + idx + "."));
                } else {
        ...
        new Thread(() -> {
            try {
                ...
                if (found) {
                    ...
                    javafx.application.Platform
                            .runLater(() -> showAlert("Invalid Index", "Cannot delete at index " + idx));
                }
            } catch (Exception e) {
            } finally {
        ...
        new Thread(() -> {
            try {
                ...
                if (!found) {
                    ...
                    javafx.application.Platform
                            .runLater(() -> showAlert("Not Found", "Value " + val + " is not present."));
                }
            } catch (Exception e) {
            } finally {
                isAnimating = false;
    ...
    private void updateVisualization() {
        ...
        nullStartLabel.setStyle(
                "-fx-font-size: 16; -fx-font-weight: bold; -fx-text-fill: white; -fx-background-color: #e74c3c; -fx-padding: 8 15; -fx-background-radius: 5; -fx-border-color: #c0392b; -fx-border-radius: 5; -fx-border-width: 2;");
        ...
        while (current != null) {
            ...
            ftNode.setFromValue(0.0);
            ftNode.setToValue(1.0);
            ...
            if (current.getNext() != null) {
                ...
                ftArr.setFromValue(0.0);
                ftArr.setToValue(1.0);
    ...
    private void highlightNode(int index, String hexColor) {
        ...
        if (childIndex < listContainer.getChildren().size()) {
            ...
            valueLabel.setStyle(oldStyle.replaceAll("-fx-background-color: #[0-9a-fA-F]{6};",
    ...
    private void showAlert(String title, String message) {
        if (title != null && (title.toLowerCase().contains("complete") || title.toLowerCase().contains("ready")
                || title.toLowerCase().contains("custom mode"))) {
