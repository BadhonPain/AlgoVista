...
public class SinglyLinkedListController {
    ...
    private void append() {
        ...
        new Thread(() -> {
            try {
                ...
                if (size > 0)
                    logStep("Found Tail node. Setting Tail's next pointer to new node...");
                else
                    logStep("List is empty. Setting Head to new node...");
                Thread.sleep((long) ((800) * com.AlgoVista.utils.SettingsManager.getSleepMultiplier()));
    ...
    private void highlightNode(int index, String hexColor) {
        ...
        if (childIndex < listContainer.getChildren().size()) {
            ...
            valueLabel.setStyle(oldStyle
                    .replaceAll("-fx-background-color: #[0-9a-fA-F]{6};", "-fx-background-color: " + hexColor + ";")
                    .replace("SkyBlue", hexColor));
